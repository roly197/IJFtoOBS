...
class Deser_ijf():
        ...
        def deserializeIjf(self, udpUpdate):
                ...
                self.eventTextData = {self.subst+'EventName':udpUpdate[4:24], 
                        ...
                        self.subst+'PinTimeW':udpUpdate[97:99], 
                        ...
                        self.subst+'PinTimeB':udpUpdate[159:161], 
                        self.subst+'TeamScoreW':udpUpdate[161]}
                self.eventVisibleData = {self.subst+'GoldenScore':self._booleanCase(udpUpdate[162]), 
                        ...
                        self.subst+'PinTimerW':self._pinTimerVisible(udpUpdate[97:99]), 
                        self.subst+'PinTimerB':self._pinTimerVisible(udpUpdate[159:161]), 
                ...
                for key in self.eventFaultData.keys():                            #Update changed visibility on Shido (1,2,3)/Hansokumake(H)
                        if self.eventFaultData[key] != self.oldEventData[key]:             #Fault (Shido/Hansokumake) data has changed
                                ...
                                if key == self.subst+'ShidoHansW':                            #Start sending faults for A
                                        data = 'SetSceneItemProperties',{'item': self.subst+'ShidoW1', 'visible': a[0]}
                                        sb_list.append(data)
                                        data = 'SetSceneItemProperties',{'item': self.subst+'ShidoW2', 'visible': a[1]}
                                        sb_list.append(data)
                                        data = 'SetSceneItemProperties',{'item': self.subst+'HansokumakeW', 'visible': a[2]}
                                        sb_list.append(data)
                                        #print ('FaultA: ' + str(a[0]), str(a[1]), str(a[2]))
                                elif self.subst+'ShidoHansB':                                 #Start sending faults for B
                                        data = 'SetSceneItemProperties',{'item': self.subst+'ShidoB1', 'visible': a[0]}
                                        sb_list.append(data)
                                        data = 'SetSceneItemProperties',{'item': self.subst+'ShidoB2', 'visible': a[1]}
                                        sb_list.append(data)
                                        data = 'SetSceneItemProperties',{'item': self.subst+'HansokumakeB', 'visible': a[2]}
                                        sb_list.append(data)
...
async def make_request_loop():
        ...
        '''Main loop. Listen to broadcast data from IJF scoreboards and run forever'''
        while True:
                recieved = sock.recvfrom(1024)
                udpUpdate = recieved[0].decode('UTF-8')
                ip = str(recieved[1][0])
                mat = udpUpdate[209]                                                  #From the received message get the ma# as int to use as a unique identifier
                ...
                if mat in scoreBoardInstances.keys():                                 # Check if an instance with mat # exists
                        ...
                        for i in scoreBoardInstances[mat].update_sb(udpUpdate):       #If exists: Iterate through the message list to be sent
